...
def unzip_to_temp_dir(zip_file_name):
    ...
    LOGGER.info(f"Extracting zipped file: {zip_file_name}")
    ...
    try:
        ...
        for zip_name in zf.namelist():
            ...
            if (name.endswith(os.path.sep) and not os.path.exists(dest)):
                os.mkdir(dest)
                LOGGER.debug(f"Directory {dest} created.")
        ...
        for zip_name in zf.namelist():
            ...
            if not (name.endswith(os.path.sep)):
                LOGGER.debug(f"Copying file {dest}......")
                with open(dest, 'wb') as outfile:
                    outfile.write(zf.read(zip_name))
                LOGGER.debug(f"File {dest} copied.")

        LOGGER.info(f"Unzipped file can be found at {tempdir}")
        return tempdir

    except IOError as err:
        LOGGER.error(f"Error in extracting webdriver.xpi: {err}")
        return None
