...
class selenium(object):
    ...
    *   \ **exact:**\ \ *string*:
    ...
    def start(self, browserConfigurationOptions=None, driver=None):
        ...
        if driver:
            start_args.append(f'webdriver.remote.sessionid={driver.session_id}')
    ...
    def do_command(self, verb, args):
        ...
        try:
            ...
            for i in range(len(args)):
                body += f'&{unicode(i+1)}=' + urllib_parse.quote_plus(
                    unicode(args[i]).encode('utf-8')
                )

            if self.sessionId != None:
                body += f"&sessionId={unicode(self.sessionId)}"
            headers = {
                "Content-Type":
