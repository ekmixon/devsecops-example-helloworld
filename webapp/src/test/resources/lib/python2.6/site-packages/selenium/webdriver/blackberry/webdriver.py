...
class WebDriver(RemoteWebDriver):
    ...
    def __init__(self, device_password, bb_tools_dir=None,
        ...
        remote_addr = f'http://{hostip}:{port}'

        filename = 'blackberry-deploy'
        if platform.system() == "Windows":
            filename += '.bat'

        if bb_tools_dir is None:
            bb_deploy_location = filename

        elif os.path.isdir(bb_tools_dir):
            bb_deploy_location = os.path.join(bb_tools_dir, filename)
            if not os.path.isfile(bb_deploy_location):
                raise WebDriverException(
                    f'Invalid blackberry-deploy location: {bb_deploy_location}'
                )

        else:
            raise WebDriverException(
                f'Invalid blackberry tools location, must be a directory: {bb_tools_dir}'
            )
