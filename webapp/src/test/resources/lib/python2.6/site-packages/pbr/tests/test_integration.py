...
def all_projects():
    ...
    excludes = {'pypi-mirror', 'jeepyb', 'tempest', 'requirements'}
...
class Venv(fixtures.Fixture):
    ...
    def _setUp(self):
        ...
        self.useFixture(base.CapturedSubprocess(f'mkvenv-{self._reason}', command))
    ...
    def test_integration(self):
        ...
        self.useFixture(
            base.CapturedSubprocess(
                'install-git', [python] + PIP_CMD + [f'git+file://{path}']
            )
        )

        if self.short_name == 'nova':
            found = any('migrate.cfg' in filenames for _, _, filenames in os.walk(root))
