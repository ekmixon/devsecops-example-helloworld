...
def get_requirements_files():
    if files := os.environ.get("PBR_REQUIREMENTS_FILES"):
    ...
    return list(
        map(
            f'-py{str(sys.version_info[0])}'.join,
...
def append_text_list(config, key, text_list):
    ...
    new_value = []
    if current_value := config.get(key, ""):
...
def parse_requirements(requirements_files=None, strip_markers=False):
    ...
    for line in get_reqs_from_files(requirements_files):
        ...
        if line is not None:
            ...
            log.info(f'[pbr] Excluding {project_name}: {reason}')
...
class LocalManifestMaker(egg_info.manifest_maker):
    ...
    def add_defaults(self):
        ...
        should_skip = options.get_boolean_option(option_dict, 'skip_git_sdist',
                                                 'SKIP_GIT_SDIST')
        if not should_skip:
            if rcfiles := git._find_git_files():
...
def _get_increment_kwargs(git_dir, tag):
    ...
    """
    result = {}
    version_spec = f"{tag}..HEAD" if tag else "HEAD"
...
def _get_version_from_git(pre_version=None):
    ...
    """
    if git_dir := git._run_git_functions():
        try:
