...
class TestWsgiScripts(base.BaseTestCase):
    ...
    def test_wsgi_script_install(self):
        ...
        stdout, _, return_code = self.run_setup('install', f'--prefix={self.temp_dir}')

        self.useFixture(
            fixtures.EnvironmentVariable(
                'PYTHONPATH',
                f".:{self.temp_dir}/lib/python{sys.version_info[0]}.{sys.version_info[1]}/site-packages",
            )
        )
    ...
    def _test_wsgi(self):
        for cmd_name in self.cmd_names:
            ...
            print(f"Running {cmd} > {stdout.name}")
            ...
            self.assertIsNotNone(m, f"Regex failed to match on {stdoutdata}")
    ...
    application = None"""
        ...
        for cmd_name in self.cmd_names:
            install_txt = f'Installing {cmd_name} script to {self.temp_dir}'
            ...
            app_name = "main" if cmd_name == 'pbr_test_wsgi' else "WSGI.app"
