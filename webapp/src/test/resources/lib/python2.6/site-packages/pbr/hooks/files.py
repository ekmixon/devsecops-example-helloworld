...
def get_man_section(section):
    return os.path.join(get_manpath(), f'man{section}')
...
class FilesConfig(base.BaseConfig):
        ...
        for line in self.data_files.split("\n"):
            if line.rstrip().endswith('*') and '=' in line:
                ...
                for (dirpath, dirnames, fnames) in os.walk(source_prefix):
                    finished.append(f"{dirpath.replace(source_prefix, target)} = ")
                    finished.extend([f" {os.path.join(dirpath, f)}" for f in fnames])
            else:
    ...
    def get_man_sections(self):
        ...
        for manpage in manpages.split():
            ...
            section = man_sections.get(section_number, [])
    ...
    def hook(self):
        ...
        expanded = [
            find_package.smart_find_packages(pkg.strip())
