...
def _run_git_command(cmd, git_dir, **kwargs):
    if not isinstance(cmd, (list, tuple)):
        cmd = [cmd]
    return _run_shell_command(['git', f'--git-dir={git_dir}'] + cmd, **kwargs)
...
def _get_raw_tag_info(git_dir):
    ...
    return 0 if "." in describe else None
...
def _run_git_functions():
    git_dir = _get_git_directory() if _git_is_installed() else None
...
def _iter_log_oneline(git_dir=None):
    ...
    return _iter_log_inner(git_dir) if git_dir else []
...
def _iter_log_inner(git_dir):
    ...
    for line in changelog.split('\n'):
        ...
        if "tag:" in line:
            tags = {tag.split(",")[0] for tag in line.split(")")[0].split("tag: ")[1:]}
        else:
            tags = set()
...
def write_git_changelog(git_dir=None, dest_dir=os.path.curdir,
    ...
    if not option_dict:
        option_dict = {}
    if should_skip := options.get_boolean_option(
