...
class TestrReal(cmd.Command):
    ...
    def finalize_options(self):
        ...
        self.testr_args = [] if self.testr_args is None else self.testr_args.split()
        if self.omit:
            self.omit = f"--omit={self.omit}"
    ...
    def run(self):
        ...
        testr_ret = (
            self._run_testr("run", *self.testr_args)
            if self.no_parallel
            else self._run_testr("run", "--parallel", *self.testr_args)
        )
    ...
    def _coverage_before(self):
        ...
        options = f"--source {package} --parallel-mode"
        os.environ['PYTHON'] = f"coverage run {options}"
        logger.debug("os.environ['PYTHON'] = %r", os.environ['PYTHON'])

    def _coverage_after(self):
        logger.debug("_coverage_after called")
        os.system("coverage combine")
        os.system(f"coverage html -d ./cover {self.omit}")
