...
class SkipFileWrites(base.BaseTestCase):
    ...
    def setUp(self):
        ...
        if not os.path.exists(self.git_dir):
            self.skipTest(f"{self.git_dir} is missing; skipping git-related checks")
            return
...
class GitLogsTest(base.BaseTestCase):
    ...
    def test_generate_authors(self):
        ...
        co_author_by = f"Co-authored-by: {co_author}"

        git_log_cmd = (
            "git --git-dir=%s log --format=%%aN <%%aE>"
            % self.git_dir)
        git_co_log_cmd = f"git --git-dir={self.git_dir} log"
    ...
    def test_parse_requirements_override_with_env_multiple_files(self):
        ...
        self.useFixture(
            fixtures.EnvironmentVariable(
                'PBR_REQUIREMENTS_FILES', f"no-such-file,{self.tmp_file}"
            )
        )
