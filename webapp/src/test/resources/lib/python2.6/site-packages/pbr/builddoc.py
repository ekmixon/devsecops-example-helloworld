...
def _find_modules(arg, dirname, files):
    for filename in files:
        if filename.endswith('.py') and filename != '__init__.py':
            arg[f"{dirname.replace('/', '.')}.{filename[:-3]}"] = True
...
class LocalBuildDoc(setup_command.BuildDoc):
    ...
    def generate_autoindex(self, excluded_modules=None):
        log.info(f"[pbr] Autodocumenting from {os.path.abspath(os.curdir)}")
        modules = {}
        ...
        module_list = sorted(mod for mod in modules if include(mod))
...
""")
            for module in module_list:
                output_filename = os.path.join(source_dir, f"{module}.rst")
                heading = f"The :mod:`{module}` Module"
                underline = "=" * len(heading)
                values = dict(module=module, heading=heading,
                              underline=underline)

                log.info(f"[pbr] Generating {output_filename}")
    ...
    def _sphinx_run(self):
        status_stream = sys.stdout if self.verbose else cStringIO.StringIO()
        confoverrides = {}
        if self.version:
        ...
        try:
            ...
            if not isinstance(err, utils.SystemMessage):
                raise
